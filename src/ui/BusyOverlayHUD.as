namespace BusyHUD {
    void RenderBusyOverlayHUD(BusyOverlay::State@ st) {
        ...
        // Detail
        string detailLine = st.detail;
        if (st.showTimer) {
            uint now = Time::Now;
            uint elapsedMs = (st.timerStartMs == 0 || now < st.timerStartMs) ? 0 : (now - st.timerStartMs);
            uint secs = elapsedMs / 1000;
            detailLine = st.timerPrefix + secs + "s";
        }

        if (detailLine.Length > 0) {
            nvg::FillColor(vec4(0.80f, 0.85f, 0.95f, 1));
            nvg::FontSize(16.0f);
            nvg::Text(px + panelW * 0.5f, py + 110.0f, detailLine);
        }
    }
}
